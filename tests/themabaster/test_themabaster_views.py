...
def test_wired_render_root_view(themester_app, this_pagecontext, html_config, sphinx_config):
    context = Site()
...
def test_wired_render_nocontext_view(themester_app, this_pagecontext, html_config, sphinx_config):
...
def test_wired_render_document_view(themester_app, this_pagecontext, html_config, sphinx_config):
    # context = Document(name='Some Document', parent=themester_app.root)
    render_container = themester_app.registry.create_container(context=None)
    render_container.register_singleton(this_pagecontext, PageContext)
    render_container.register_singleton(sphinx_config, SphinxConfig)
